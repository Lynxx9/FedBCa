...
parser.add_argument('-FL mode', '--mode', type=str, default="FedAvg", help="FedProx, FedBN, siloBN, FedAvg")
...
# GPU id selected
parser.add_argument('-g', '--gpu', type=str, default='0', help='gpu id to use(e.g. 0,1)')
...
# 模型名称
parser.add_argument('-mn', '--model_name', type=str, default='Resnet2d', help='the model to train')
# clients selected
...
parser.add_argument('-sp', '--save_path', type=str,
                    default='/home/yuihsin/FedBCa/results/',
...
parser.add_argument('--leave_out_center', type=int, default=4,
                    help='LOCO: center used only for testing, choose 1,2,3,4')
...
if __name__ == "__main__":
    "data path"
    Center1_h5data_path = "/home/yuihsin/FedBCa/process_classification/Center1/h5_data_128"
    Center1_train_excelpath = "/home/yuihsin/FedBCa/process_classification/Center1/center1_train.csv"
    Center1_test_excelpath = "/home/yuihsin/FedBCa/process_classification/Center1/center1_test.csv"

    Center2_h5data_path = "/home/yuihsin/FedBCa/process_classification/Center2/h5_data_128"
    Center2_train_excelpath = "/home/yuihsin/FedBCa/process_classification/Center2/center2_train.csv"
    Center2_test_excelpath = "/home/yuihsin/FedBCa/process_classification/Center2/center2_test.csv"

    Center3_h5data_path = "/home/yuihsin/FedBCa/process_classification/Center3/h5_data_128"
    Center3_train_excelpath = "/home/yuihsin/FedBCa/process_classification/Center3/center3_train.csv"
    Center3_test_excelpath = "/home/yuihsin/FedBCa/process_classification/Center3/center3_test.csv"

    Center4_h5data_path = "/home/yuihsin/FedBCa/process_classification/Center4/h5_data_128"
    Center4_train_excelpath = "/home/yuihsin/FedBCa/process_classification/Center4/center4_train.csv"
    Center4_test_excelpath = "/home/yuihsin/FedBCa/process_classification/Center4/center4_test.csv"
    ...
    Test_center_AUC_list = [[]]   # 只存 LOCO test center
    train_loss_list = []
    train_lr_list = []

    Test_client_list = [f"client{args['leave_out_center']}"]
    ...
    test_center_list = [np.array(["Epoch","ACC","SEN","SPEC","PREC","Threshold","AUC"])]
    ...
    for i in range(args['num_comm']):
        ...
        if args["whether_random_training"]:
            ...
            "LOCO"
            leave_out = args["leave_out_center"]

            clients_in_comm = [
                f"client{i}" for i in range(1, args['num_of_clients'])
                if i != leave_out
            ]

            print("LOCO train clients:", clients_in_comm)
            print("LOCO test client:", f"client{leave_out}")
            client_weights = [1.0 / len(clients_in_comm)] * len(clients_in_comm)
                ...
                for k, client in enumerate(tqdm(clients_in_comm)):
                    ...
                    if args["mode"].lower() == 'fedprox':
                        if i > 0:
                            ...
                            current_lr, loss_value, local_parameters = train_step_cls(train_loader=Center_train_loader[client],
        ...
        if Start_test:
            ...
            n_test = len(Test_client_list)
            ACC_value_avg = same_commo_ACC / n_test
            SEN_value_avg = same_commo_SEN / n_test
            SPE_value_avg = same_commo_SPC / n_test
            AUC_value_avg = same_commo_AUC / n_test
            Test_all_AUC_list.append(AUC_value_avg)
            PREC_value_avg = same_commo_PREC / n_test
            ...
            if args["record_data"]:
                if args["num_class"] == 2:
                    ...
                    test_txt.write("*"*10 + f"AUC result for test data for LOCO center {args['leave_out_center']}" + "*"*10 + "\n" +
                                    "AUC: " + str(AUC_value) + "\n" +
                                    'ACC: ' + str(ACC_value) + "\n" +
                                    "SEN: " + str(SEN_value) + "\n" +
                                    "SPE: " + str(SPE_value) + "\n" +
